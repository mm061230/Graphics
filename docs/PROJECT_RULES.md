...
- Each task under a source image owns its own `input/` and `final/` directories.
...
- Keep GATE 1 candidate JSON, review marks, state tokens, and failure reports in `work/`, not in `result/`.
